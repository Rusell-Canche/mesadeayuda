...
public function storeTicket(Request $request)
{
    $validatedData = $request->validate([
        ...
        'archivo' => 'nullable|file|mimes:pdf,doc,docx,jpg,jpeg,png|max:5120', // 5MB max
    ]);
    ...
    try {
        ...
        // Agregar ID del usuario que crea el ticket (opcional)
        $validatedData['user_id'] = Auth::id();

        // Crear ticket
        $ticket = Ticket::create($validatedData);

        return response()->json(['message' => 'Ticket creado con éxito', 'data' => $ticket], 201);
